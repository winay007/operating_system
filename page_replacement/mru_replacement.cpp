...
void mru_replacement(int *reference_string, int n, int frame_size)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        else
        {
            ...
            int most_recent = occurence[0], idx = 0;
            for (int j = 1; j < frame_size; j++)
            {
                if (occurence[j] > most_recent)
                {
                    most_recent = occurence[j];
                    idx = j;
                }
            }
            page_faults++;
...
int main()
{
    ...
    mru_replacement(reference_string, n,frame_size);
    return 0;
}